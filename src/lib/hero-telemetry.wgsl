...
fn agentComputer(p: vec2f) -> f32 {
  let q = rotate(p, -0.04);
  let shell = sdRoundedBox(q, vec2f(0.20, 0.135), 0.025);
  let screen = sdRoundedBox(q - vec2f(0.0, -0.012), vec2f(0.168, 0.098), 0.012);
  var mark = opSmoothSub(shell, screen, 0.008);
  let stem = sdRoundedBox(q - vec2f(0.0, 0.16), vec2f(0.018, 0.045), 0.008);
  let base = sdRoundedBox(q - vec2f(0.0, 0.205), vec2f(0.095, 0.012), 0.006);
  mark = opSmoothUnion(mark, stem, 0.006);
  mark = opSmoothUnion(mark, base, 0.006);
  return mark;
}
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let green = vec3f(0.082353, 0.650980, 0.521569);
  ...
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let col = mix(green, paper, spark * 0.45 + line * 0.05);
